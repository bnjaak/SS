...
" 
Write-Host "";
Write-Host -ForegroundColor Red "Este es el BamParser de Brr Br patapin, usalo con cuidado o si no te pega."; -NewLine
Write-Host "";

function Test-Admin {;$currentUser = New-Object Security.Principal.WindowsPrincipal $([Security.Principal.WindowsIdentity]::GetCurrent());$currentUser.IsInRole([Security.Principal.WindowsBuiltinRole]::Administrator);}
if (!(Test-Admin)) {
    Write-Warning "Brr Brr patapin te va a pegar si no lo ejecutas como Admin"
    Start-Sleep 10
    Exit
}
...
Catch{
    Write-Warning "Error Parseando BAM Key. Probablemente no soporta tu version de Windows, Patapin se enojo >:("
    Exit
}
...
$Bam = Foreach ($Sid in $Users){$u++
        ...
        foreach($rp in $rpath){
           ...
           Write-Host -ForegroundColor DarkYellow "Sahur esta extrayendo " -NoNewLine
...
Write-Host ""
Write-Host "Se tardo $t Minutos" -ForegroundColor DarkYellow